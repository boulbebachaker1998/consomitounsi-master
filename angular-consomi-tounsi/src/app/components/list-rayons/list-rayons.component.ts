...
export class ListRayonsComponent implements OnInit {
  ...
  filterRayons(rayons: Rayon[]) {
    return rayons.filter((e) => {
      return e.rayonName.toLowerCase().includes(this.filters.keyword.toLowerCase());
    }).sort((a, b) => {
      if (this.filters.sortBy === 'rayonName') {
        return a.rayonName.toLowerCase() < b.rayonName.toLowerCase() ? -1: 1;
